...
- [ ] **"Add to Calendar" button** — add a calendar option inside the event detail modal. Address only included if `address_public = true`, city only if private. Consider two options: (1) ICS file download — works on iPhone seamlessly, Android requires finding the file in Downloads, desktop Google Calendar users need to import manually which is not obvious. (2) Direct Google Calendar link — one click, opens pre-filled, works great on Android and desktop but iPhone users may prefer ICS. Best solution: offer both — a Google Calendar button and an ICS download — to cover all devices cleanly.
- [x] **Location field UI improvement** — redesigned as a clear In Person / Online toggle matching the Free/Paid UI style. City dropdown only appears for in-person. *(Session 12, Mar 11 2026)*
...
- [x] **Image size limit + Storage cleanup** — 2MB hard limit on upload enforced in both submit and edit forms with clear error message. Images now deleted from Supabase Storage on all delete paths. *(Session 14, Mar 14 2026)*
...
- [x] **Tagline under site name** — added beneath "Atlanta IL Community Events" in the header.
- [ ] **Phone number format validation** — confirm edge cases (international numbers, paste).
- [x] **Repeat organizer memory** — added autocomplete="name" and autocomplete="email" to organizer fields. Works on desktop browsers. iPhone Safari does not support this reliably for non-login forms. *(Session 13, Mar 13 2026)*
...
## Completed
- [x] **Email retry on cold start** — sendEmail now retries once after 3 seconds if the first attempt fails. Fixes silent email loss when Edge Function is waking up from idle. *(Session 15, Mar 15 2026)*
- [x] **Email gold bar width on mobile** — fixed gold accent bar stretching too wide on iPhone. Locked to 4px with min/max-width. Requires Edge Function redeploy. *(Session 15, Mar 15 2026)*
